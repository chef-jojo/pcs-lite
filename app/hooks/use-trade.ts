...
export function useAllCommonPairs(
  ...
  const basePairs: (readonly [Token, Token])[] = useMemo(
    () =>
      bases.flatMap((base) =>
        bases.map((base2) => [base, base2] as const),
      ),
    [bases],
  );
...
export function useIsTransactionUnsupported(
  ...
  if (unsupportedTokens) {
    if (
      tokenIn &&
      Object.keys(unsupportedTokens).includes(tokenIn.address)
    ) {
      return true;
    }
    if (
      tokenOut &&
      Object.keys(unsupportedTokens).includes(tokenOut.address)
    ) {
      return true;
    }
  }
